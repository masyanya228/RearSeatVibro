...
//0-left; 1-right
void ClickHardware(uint8_t seatNum){
  if(seatNum==0)
    seatL.mode=GetNextMode(seatL.mode);
  else if(seatNum==1)
    seatR.mode=GetNextMode(seatR.mode);
}
...
uint8_t GetNextMode(uint8_t mode){
  int i=0;
  while(i<4){
    if(modeSeq[i]==mode){
      break;
